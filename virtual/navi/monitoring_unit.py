...
class Agv_Car(QPoint):
    ...
    def get_information_json(self):
        information = {}
        information["id"] = str(self.__id)
        information["location_x"] = str(self.__location.x())
        information["location_y"] = str(self.__location.y())
        information["angle"] = str(round(self.__angle, 1))    # round(a,2)

        information["show_location_x"] = str(self.show_location_x)
        information["show_location_y"] = str(self.show_location_y)
        information["show_angle"] = self.show_angle  # round(a,2)
        information["show_goal_x"] = str(self.show_goal_x)  # round(a,2)
        information["show_goal_y"] = str(self.show_goal_y)  # round(a,2)
        information["line_speed"] = self.__line_speed
        information["angle_speed"] = self.__angle_speed
        information["goal"] = self.__goal
        information["show_goal_x"] = self.show_goal_x  # round(a,2)
        information["show_goal_y"] = self.show_goal_y  # round(a,2)
        information["carry_cargo"] = self.__carry_cargo
        information["power"] = str(100)
        return information

    def _create_car_image(self):
        # 绘制一个小车
        self.__image = QImage(40, 40, QImage.Format_ARGB32)
    ...
    def init_json(self, data):  # data 列表
        ...
        with open(filename, 'r+', encoding='utf-8') as f:
            ...
            for i in range(0, number):
                index = str(i)
                print(load_dict.get(index).get("type"))
